...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        if not len(image.shape) == 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray image with 2 dims.\n"))
...
class Resize(object):
    ...
    def __call__(self, image, label):
        ...
        back_crop = np.zeros((test_size, test_size)) 
...
class Crop(object):
    ...
    def __call__(self, image, label):
        ...
        if image.shape != (self.size[0], self.size[0]):
