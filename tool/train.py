...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    train_data = dataset.SemData(data_root=args.data_root, transform=train_transform, mode='train')
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform = transform.Compose([
                ...
                transform.ToTensor(),
                transform.Normalize(mean=mean, std=std)])           
        val_data = dataset.SemData(data_root=args.data_root, transform=val_transform, mode='val')
        val_sampler = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if main_process():
            ...
            writer.add_scalar('FBIoU_train', FBIoU_train, epoch_log)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, cls) in enumerate(train_loader):
